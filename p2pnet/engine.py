    ...
    for idx in range(samples.shape[0]):
        ...
        if des is not None:
            cv2.imwrite(
                os.path.join(
                    vis_dir,
                    "{}_{}_gt_{}_pred_{}_gt.jpg".format(
                        int(name), des, len(gts[idx]), len(pred[idx])
                    ),
                ),
                sample_gt,
            )
            cv2.imwrite(
                os.path.join(
                    vis_dir,
                    "{}_{}_gt_{}_pred_{}_pred.jpg".format(
                        int(name), des, len(gts[idx]), len(pred[idx])
                    ),
                ),
                sample_pred,
